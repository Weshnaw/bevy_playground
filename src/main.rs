...
fn main() {
    App::new()
        .add_plugins((
            DefaultPlugins,
            GpuReadbackPlugin,
            ExtractResourcePlugin::<UniformBuffer>::default(),
...
struct UniformBuffer(Handle<ShaderStorageBuffer>);
...
fn setup(
    ...
    // Create a storage buffer with some data
    let uniform = 0u32;
    let mut uniform = ShaderStorageBuffer::from(uniform);
    uniform.buffer_description.usage |= BufferUsages::UNIFORM;
    let uniform = buffers.add(uniform);
    commands.insert_resource(UniformBuffer(uniform));
...
fn prepare_bind_group(
    ...
    let uniform = buffers.get(&uniform.0).unwrap();
    ...
    let bind_group = render_device.create_bind_group(
        ...
        &BindGroupEntries::sequential((
            ...
            uniform.buffer.as_entire_buffer_binding(),
        )),
    );
